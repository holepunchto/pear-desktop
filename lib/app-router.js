      ...
      for (const a of fragments) {
        if (a.getAttribute('href') === target) {
          if (a.parentElement.tagName === 'SPAN') {
            const link = a.parentElement.querySelector('span ~ a')
            link.textContent = href[0] === '?' ? href.slice(1) : href
  ...
  class AppRouter extends HTMLElement {
    ...
    remap(page) {
      const map = {
        ...
        migration: 'vendor/pear-docs/reference/migration',
      ...
      if (pathname.startsWith('/vendor/pear-docs') && pathname.includes('.') === false) {
        pathname += '.md'
      }
    ...
    async load(pathname = '/', opts = {}) {
      ...
      if (this.page === pathname && this.fragment === opts.fragment && this.query === opts.query)
        return
      this.page = pathname
      ...
      for (const [route, element] of Object.entries(this.routes)) {
        if (pathname === route || pathname.startsWith('/documentation')) {
          ...
          if (!opts.back)
            history.pushState(
              { pathname, fragment: opts.fragment, query: opts.query },
              null,
              this.nav.active
            )
          break
    ...
    link(evt) {
      ...
      const pathname = evt.target.pathname.replace(Pear.app.route, '') // remove coldStart entry so it doesnt mess with routes
      this.load(route + pathname, {
        fragment: evt.target.hash.slice(1),
        query: evt.target.search.slice(1)
      }).catch(console.error)
    }
    ...
    connectedCallback() {
      ...
      window.addEventListener('popstate', (evt) => {
        this.load(evt.state?.pathname, {
          back: true,
          fragment: evt.state?.fragment,
          query: evt.state?.query
        }).catch(console.error)
      })

      window.addEventListener('load', () => {
        const page = this.remap(Pear.app.route || '/')
        const opts = { fragment: Pear.app.fragment, query: Pear.app.query }
        this.load(page, opts).catch(console.error)
        Pear.wakeups(({ entrypoint, query, fragment }) => {
          ui.app.focus({ steal: true }).catch(console.error)
          this.load(entrypoint, { query, fragment }).catch(console.error)
        })
      })
    }
  }
