...
function expandWinPath (path) {
  ...
  do {
    prev = path
    path = path.replaceAll(/%([^%]+)%/g, (match, name) => process.env[name.toUpperCase()] || match)
  } while (path !== prev)
...
customElements.define('system-status', class extends HTMLElement {
    ...
    <div id="panel">
      ...
      ${
        ...
        ? (isActivePearCorrect
             ...
             : `
              <blockquote>
                <p>Pear is in the ${pathLocation} PATH but not being used as the main path${isWin && isActivePearInSystemPath ? ' because it is being overridden by a different executable in the system path' : ''}.</p>
              </blockquote>
              <p>To attempt to fix this, click the button below.</p>
              <p><button id="fix-path-button">${isWin ? (isActivePearInSystemPath ? 'Reinstall Pear to the system PATH' : 'Reinstall Pear to the user PATH') : 'Reinstall Pear'}</button><p>
             `
          )
        : `
          <blockquote>
  ...
  #installed () {
    ...
    if (isWin) {
      // TODO: See if we should only be expanding the windows path if the key type if REG_EXPAND_SZ
