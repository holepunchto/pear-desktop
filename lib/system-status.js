...
customElements.define('system-status', class extends HTMLElement {
    ...
    <div id="panel">
      ...
      ${
        ...
        : `
          ...
          <p><code>${BIN}</code></p>
          ${isWin || isCurrentShellSupported ? '<p> or click the button.</p> <p><button id="setup-button">Automatic Setup Completion</button><p>' : ''}
        `
      }
    </div>
    `
  ...
  #installPath () {
    if (isWin) {
      const npmPearExec = which.sync('pear', { nothrow: true })
