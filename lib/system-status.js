...
customElements.define('system-status', class extends HTMLElement {
  ...
  async #install () {
    ...
    if (isWin) {
      const runtimeAbsPath = path.join(os.homedir(), 'AppData', 'Roaming', 'pear', runtimeSubpath)
      ...
      if (!fs.existsSync(pearPs1)) fs.writeFileSync(path.join(BIN, 'pear.ps1'), `& "${runtimeAbsPath}" @args`)

      const originalPath = spawnSync('reg', ['query', 'HKCU\\Environment', '/v', 'Path'], { stdio: 'pipe' })
        .stdout.toString()?.match(/REG_SZ\s+([^\r\n]+)\r?\n/)?.[1]
      if (!originalPath.includes(BIN)) {
        spawnSync('reg', ['add', 'HKCU\\Environment', '/v', 'Path', '/t', 'REG_SZ', '/d', `${BIN};${originalPath}`, '/f'], { stdio: 'inherit' })
      }
    } else {
