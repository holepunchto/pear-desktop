...
function getWinPath () {
  ...
  if (!Array.isArray(userPathMatches) || userPathMatches.length !== 3) {
    throw new Error('Failed to extract user PATH')
  }

  const userPathType = userPathMatches[1]
  let userPath = userPathMatches[2]
  ...
  if (!Array.isArray(systemPathMatches) || systemPathMatches.length !== 3) {
    throw new Error('Failed to extract system PATH')
  }

  const systemPathType = systemPathMatches[1]
  let systemPath = systemPathMatches[2]
  if (systemPath.startsWith('"') && systemPath.endsWith('"')) {
    systemPath = systemPath.slice(1, -1).replaceAll('""', '"')
  }

  return { userPath, userPathType, systemPath, systemPathType }
}
...
customElements.define('system-status', class extends HTMLElement {
  ...
  #installed () {
    ...
    if (isWin) {
      const { userPath, userPathType, systemPath, systemPathType } = getWinPath()
      expandedUserPath = userPathType === 'REG_EXPAND_SZ' ? expandWinPath(userPath) : userPath
      expandedSystemPath = systemPathType === 'REG_EXPAND_SZ' ? expandWinPath(systemPath) : systemPath
