...
function writeWinReg (dir, key, value, admin = false) {
  ...
  const tempRegPath = path.join(os.tmpdir(), `${fileName}.reg`)
...
customElements.define('system-status', class extends HTMLElement {
      ...
      ${
        ...
        ? (isActivePearCorrect
             ...
             : `
              ...
              <p><button id="fix-path-button">${isWin ? (isActivePearInSystemPath ? 'Reinstall Pear to the system PATH (requires administrator access)' : 'Reinstall Pear to the user PATH') : 'Reinstall Pear to the end of the shell rc file'}</button><p>
             `
          )
        : `
          <blockquote>
  ...
  #install (toSystemPath = false) {
    ...
    if (isWin) {
      ...
      const { userPath, systemPath } = getWinPath()
      if (toSystemPath) {
        const newPath = `${BIN}${path.delimiter}${systemPath}`
  ...
  #fixPath () {
    if (isWin) {
      ...
      const userNpmPearIndex = expandedSplitUserPath.indexOf(npmPearPath)
      ...
      const systemNpmPearIndex = expandedSplitSystemPath.indexOf(npmPearPath)
      ...
      if (systemNpmPearIndex !== -1) {
        ...
        splitSystemPath.splice(systemNpmPearIndex - (systemBinIndex < systemNpmPearIndex ? 1 : 0), 0, BIN)
      ...
      if (userNpmPearIndex === -1 && systemNpmPearIndex === -1) {
        throw new Error('Failed to find overriding pear in PATH')
      }
