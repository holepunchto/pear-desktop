...
function getWinPath () {
  const userPathQuery = spawnSync('reg', ['query', 'HKCU\\Environment', '/v', 'PathTest'])
  ...
  const systemPathQuery = spawnSync('reg', ['query', 'HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment', '/v', 'PathTest'])
...
customElements.define('system-status', class extends HTMLElement {
  ...
  #install (toSystemPath = false) {
    ...
    if (isWin) {
      ...
      const { userPath, systemPath } = getWinPath()
      if (toSystemPath) {
        const escapedNewPath = `${BIN}${path.delimiter}${systemPath}`.replace(WIN_ESCAPE_MATCHER, (match) => `\\${match}`)
        const setSystemResult = spawnSync('powershell', ['-Command', `Start-Process -Verb RunAs reg -ArgumentList 'add "HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment" /v PathTest /t REG_SZ /d "${escapedNewPath}" /f'`])
        ...
        spawnSync('reg', ['add', 'HKCU\\Environment', '/v', 'PathTest', '/t', 'REG_SZ', '/d', newPath, '/f'])
      }
    } else {
  ...
  #fixPath () {
    if (isWin) {
      let npmPearExec
      try {
        npmPearExec = which.sync('pear')
      } catch (err) {
        throw new Error('Failed to find npm pear path')
      }
      const npmPearPath = path.dirname(npmPearExec)
      ...
      if (systemNpmPearIndex !== -1) {
        ...
        const escapedNewPath = newPath.replace(WIN_ESCAPE_MATCHER, (match) => `\\${match}`)
        const setSystemResult = spawnSync('powershell', ['-Command', `Start-Process -Verb RunAs reg -ArgumentList 'add "HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment" /v PathTest /t REG_SZ /d "${escapedNewPath}" /f'`])
