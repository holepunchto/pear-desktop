...
function getWinPath () {
  ...
  const systemPathQuery = spawnSync('reg', ['query', 'HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment', '/v', 'Path'])
...
function getUnixPath () {
  ...
  const lines = path.stdout.toString().split('\n').filter((line) => !!line.trim())
  const lastLine = lines?.[lines.length - 1]

  return lastLine?.trim()
}
customElements.define('not-found', class extends HTMLElement {
  connectedCallback () {
  }
...
customElements.define('system-status', class extends HTMLElement {
  ...
  constructor () {
    ...
    this.statementMatcher = new RegExp(`\n?(${escapeRegex(this.statementComment)})?${escapeRegex(this.statement)}`, 'g')
    this.root = this.attachShadow({ mode: 'open' })
    this.update = null
    this.#render()
  }
  ...
  #installPath () {
    if (isWin) {
      ...
      if (systemNpmPearIndex !== -1 && systemBinIndex !== -1 && systemBinIndex < systemNpmPearIndex) return
      ...
      if (systemNpmPearIndex !== -1) {
        ...
        const setSystemResult = writeWinReg('HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment', 'Path', newPath, true)
