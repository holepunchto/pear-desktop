...
customElements.define('system-status', class extends HTMLElement {
  ...
  #render () {
    ...
    const isCurrentShellSupported = SUPPORTED_SHELLS.some((shell) => process.env.SHELL?.endsWith(`/${shell}`))
    ...
    <div id="panel">
      ...
      ${
        ...
        : `
          ...
          <p><code>${BIN}</code></p>
          ${isWin || isCurrentShellSupported ? '<p> or click the button.</p> <p><button id="setup-button">Automatic Setup Completion</button><p>' : `As your shell (${process.env.SHELL}) is not supported, please add the above to your PATH in your shell rc file manually`}
        `
      }
    </div>
    `
