...
const REGEX_ESCAPE_MATCHER = /[-/\\^$.*+?()[\]{}|]/g
const SUPPORTED_SHELLS = ['bash', 'zsh', 'sh']

const isWin = process.platform === 'win32'

function escapeRegex(str) {
  return str.replace(REGEX_ESCAPE_MATCHER, '\\$&')
}
  ...
  try {
    result = admin
      ? spawnSync('powershell', [
          '-Command',
          `Start-Process -Wait -Verb RunAs reg -ArgumentList 'import "${tempRegPath}"'`
        ])
...
function getWinPath() {
  const userPathQuery = spawnSync('reg', ['query', 'HKEY_CURRENT_USER\\Environment', '/v', 'Path'])
  ...
  const systemPathQuery = spawnSync('reg', [
    'query',
  ...
  if (
    systemPathQuery.status !== 0 &&
    !systemPathQuery.stderr?.toString().includes('unable to find')
  ) {
...
function getUnixPath() {
  ...
  const path = spawnSync(shell, ['-ic', 'echo $PATH'])
  if (path.error || path.status !== 0 || !path.stdout) {
    throw new Error('Failed to get user PATH')
  }

  const lines = path.stdout
    .toString()
    .split('\n')
...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    load() {
      this.notFoundText.innerHTML = `${this.getAttribute('page')} not found`
      this.style.display = ''
    }
    ...
    `
      ...
      this.notFoundText = this.shadowRoot.querySelector('#not-found-text')
    }
  }
)
...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    constructor() {
      ...
      this.statementMatcher = new RegExp(
        `\n?(${escapeRegex(this.statementComment)})?${escapeRegex(this.statement)}`,
        'g'
      )
      this.root = this.attachShadow({ mode: 'open' })
      this.update = null
    ...
    #render() {
      ...
      const isCurrentShellSupported = !isWin
        ? SUPPORTED_SHELLS.some((supportedShell) => shell?.endsWith(`/${supportedShell}`))
        : false
        ...
        isPearInPath
          ? isActivePearCorrect
            ...
            : `
              ...
              <p>To fix your setup, click the button below.</p>
              <p>
          ...
          <p>To finish installing Pear Runtime, add the following to your PATH</p>
          <p><code>${BIN}</code></p>
          <p>
            ${isWin ? `or click the button below to install it to your ${pathLocation} PATH.` : isCurrentShellSupported ? `or click the button to install it to your shell (${shell}) rc file.` : `Your shell ${shell ? `(${shell}) is not supported` : 'cannot be identified'} for automatic setup. Please add the above to your PATH in your shell rc file manually.`}
          </p>
    ...
    #installed() {
      ...
      return {
        ...
        isActivePearInSystemPath: activePear
          ? expandedSystemPath?.split(path.delimiter).includes(path.dirname(activePear))
          : false
      }
    }

    #install() {
      const runtimeSubpath = path.join(
        'current',
        'by-arch',
      ...
      if (isWin) {
        ...
        if (!fs.existsSync(pearPs1))
          fs.writeFileSync(
            path.join(BIN, 'pear.ps1'),
            `& "$PSScriptRoot\\..\\${runtimeSubpath}" @args`
          )
      } else {
    ...
    #installPath() {
      if (isWin) {
        ...
        if (
          systemNpmPearIndex !== -1 &&
          systemBinIndex !== -1 &&
          systemBinIndex < systemNpmPearIndex
        )
          return

        if (systemNpmPearIndex !== -1) {
          if (systemBinIndex !== -1) splitSystemPath.splice(systemBinIndex, 1)
          splitSystemPath.splice(systemNpmPearIndex, 0, BIN)

          const newPath = splitSystemPath.join(';')

          const setSystemResult = writeWinReg(
            'HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment',
            'Path',
            newPath,
            true
        ...
        const setUserResult = writeWinReg('HKEY_CURRENT_USER\\Environment', 'Path', newPath)
      ...
      const shell = process.env.SHELL || '/bin/sh'
      const profile = shell.endsWith('/zsh')
        ? '.zshrc'
        : shell.endsWith('/bash')
          ? '.bashrc'
          : '.profile'
      ...
      if (!content.endsWith(fullStatement)) {
        const newContent = content.replace(this.statementMatcher, '') + fullStatement
