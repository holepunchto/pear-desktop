    ...
    <div id="panel">
      ...
      ${
             ...
             : `
              ...
              <p>
                <button id="fix-path-button">
                  ${isWin ? (isActivePearInSystemPath ? 'Reinstall Pear to the system PATH (as Administrator)' : 'Reinstall Pear to the user PATH') : `Reinstall Pear to the end of the shell (${shell}) rc file`}
                </button>
              </p>
             `
          )
        : `
          ...
          ${isWin || isCurrentShellSupported ? `<p><button id="setup-button">Automatic Setup Completion${isActivePearInSystemPath ? ' (as Administrator)' : ''}</button><p>` : ''}
        `
      }
    </div>
    `
