...
function writeWinReg (dir, key, value, admin = false) {
  ...
  try {
    ...
    throw new Error(`Failed to write to registry path ${path} with key ${key}`)
  } finally {
  ...
  #install (toSystemPath = false) {
    ...
    if (isWin) {
      ...
      if (toSystemPath) {
        ...
        const setSystemResult = writeWinReg('HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment', 'PathTest', newPath, true)
        ...
        const setUserResult = writeWinReg('HKCU\\Environment', 'PathTest', newPath)
