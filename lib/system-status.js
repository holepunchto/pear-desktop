...
function getUnixPath () {
  const path = spawnSync(process.env.SHELL, ['-c', 'echo $PATH'])
  if (path.status !== 0) {
...
customElements.define('system-status', class extends HTMLElement {
    ...
    <div id="panel">
      ...
      ${
        ...
        ? (isActivePearCorrect
             ...
             : `
              <blockquote>
                <p>Pear is in the system PATH but not being used as the main path.</p>
              </blockquote>
              <p>To attempt to fix this, click the button below.</p>
              <p><button id="fix-path-button"> Fix Pear PATH </button><p>
             `
          )
        : `
          <blockquote>
  ...
  #installed () {
    ...
    if (isWin) {
      ({ userPath, systemPath } = getWinPath())
      process.env.PATH = `${systemPath}${path.delimiter}${userPath}`
    } else {
      process.env.PATH = getUnixPath()
    }

    const pears = which.sync(isWin ? 'pear.cmd' : 'pear', { all: true })
    const activePear = pears?.[0]

    return {
      isPearInPath: pears.some((pear) => path.dirname(pear) === BIN),
      isActivePearCorrect: path.dirname(activePear) === BIN,
      isActivePearInSystemPath: systemPath?.split(path.delimiter).includes(path.dirname(activePear))
    }
  }
  ...
  #fixPath () {
    if (isWin) {
      ...
      if (systemNpmPearIndex !== -1) {
        ...
        const escapedNewPath = newPath.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/'/g, "\\'")
        const setSystemResult = spawnSync('powershell', ['-Command', `Start-Process -Verb RunAs reg -ArgumentList 'add "HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment" /v Pathos /t REG_SZ /d "${escapedNewPath}" /f'`])
    ...
    for (const profile in this.shellProfiles) {
      const profilePath = this.shellProfiles[profile].filepath
