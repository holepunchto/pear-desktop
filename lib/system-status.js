...
function expandWinPath (path) {
  const expandedPath = spawnSync('cmd', ['/c', 'echo', path])
  if (expandedPath.status !== 0) {
    throw new Error('Failed to expand path')
  }

  return expandedPath?.stdout?.toString()?.trim()
}
...
function getWinPath () {
  ...
  if (userPath.startsWith('"') && userPath.endsWith('"')) userPath = userPath.slice(1, -1)
  userPath.replaceAll('""', '"')
  ...
  if (systemPath.startsWith('"') && systemPath.endsWith('"')) systemPath = systemPath.slice(1, -1)
  systemPath.replaceAll('""', '"')
    ...
    <div id="panel">
      ...
      ${
        ...
        ? (isActivePearCorrect
             ...
             : `
              <blockquote>
                <p>Pear is in the ${pathLocation} PATH but not being used as the main path.</p>
              </blockquote>
