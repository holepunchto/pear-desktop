...
customElements.define('system-status', class extends HTMLElement {
  ...
  #installed () {
    if (isWin === false) {
      ...
      const currentPath = cp.spawnSync('reg', ['query', 'HKCU\\Environment', '/v', 'Path'])
  ...
  #install () {
    ...
    if (isWin) {
      ...
      const pathQuery = cp.spawnSync('reg', ['query', 'HKCU\\Environment', '/v', 'Path'])
      ...
      if (!originalPath || !originalPath.includes(BIN)) {
        ...
        cp.spawnSync('reg', ['add', 'HKCU\\Environment', '/v', 'Path', '/t', 'REG_SZ', '/d', newPath, '/f'])
      }
    } else {
